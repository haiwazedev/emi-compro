export const articleCategories = [
  "All",
  "Press Release",
  "Insight",
  "CSR News",
  "Energy News",
] as const;

export type ArticleCategory = (typeof articleCategories)[number];
export type ArticleMediaTone = "blue" | "green" | "amber" | "slate" | "navy";

export const articleMediaToneClassNames: Record<ArticleMediaTone, string> = {
  amber: "article-media-amber",
  blue: "article-media-blue",
  green: "article-media-green",
  navy: "article-media-navy",
  slate: "article-media-slate",
};

export type ArticleContentBlock =
  | { text: string; type: "paragraph" }
  | { text: string; type: "quote" }
  | { text: string; type: "heading" };

type PublishedArticleCategory = Exclude<ArticleCategory, "All">;

export type Article = {
  author: string;
  category: PublishedArticleCategory;
  content: readonly ArticleContentBlock[];
  date: string;
  dateTime: string;
  description: string;
  id: string;
  mediaTone: ArticleMediaTone;
  readTime: string;
  title: string;
};

const placeholderArticleContent = [
  {
    text: "JAKARTA — New agreements bring renewable energy certificates to Indonesia's fastest-growing digital infrastructure sector. This article body is a CMS-managed template: the hero image, category, title, metadata, and rich-text content below are all populated from the content management system.",
    type: "paragraph",
  },
  {
    text: "PT Energy Management Indonesia (Persero) continues to strengthen its position as a full-service sustainability partner, integrating Renewable Energy Certificates and SPE-GRK into comprehensive strategies for clients across the archipelago.",
    type: "paragraph",
  },
  {
    text: "Every engagement is a step toward a cleaner, more sustainable future for Indonesia.",
    type: "quote",
  },
  {
    text: "What this means for clients",
    type: "heading",
  },
  {
    text: "Organizations gain a single accountable partner across decarbonization, environmental compliance, and energy management — with outcomes measured, verified, and reported to global standards.",
    type: "paragraph",
  },
  {
    text: "For media inquiries, contact EMI Corporate Communications at pemasaran@emipersero.co.id.",
    type: "paragraph",
  },
] satisfies readonly ArticleContentBlock[];

const articleDefaults = {
  author: "Korkom PLN EMI",
  content: placeholderArticleContent,
  readTime: "4 min read",
} satisfies Pick<Article, "author" | "content" | "readTime">;

export const articles: Article[] = [
  {
    ...articleDefaults,
    category: "Press Release",
    date: "12 Jul 2026",
    dateTime: "2026-07-12",
    description:
      "New agreements bring renewable energy certificates to Indonesia's fastest-growing digital infrastructure sector.",
    id: "emi-expands-rec-coverage",
    mediaTone: "blue",
    title: "EMI Expands REC Coverage for Data Center Clients Nationwide",
  },
  {
    ...articleDefaults,
    category: "Energy News",
    date: "10 Jul 2026",
    dateTime: "2026-07-10",
    description:
      "A practical guide to Sertifikat Pengurangan Emisi and how it fits your Scope 1–3 strategy.",
    id: "understanding-spe-grk",
    mediaTone: "green",
    title: "Understanding SPE–GRK: Indonesia's Carbon Unit Explained",
  },
  {
    ...articleDefaults,
    category: "CSR News",
    date: "6 Jul 2026",
    dateTime: "2026-07-06",
    description:
      "The Ombilin fly-ash fertilizer pilot on paddy fields shows the circular-economy value of zero waste.",
    id: "faba-demplot-program",
    mediaTone: "amber",
    title: "FABA Demplot Program Boosts Rice Yields in Sawahlunto",
  },
  {
    ...articleDefaults,
    category: "Insight",
    date: "1 Jul 2026",
    dateTime: "2026-07-01",
    description:
      "From SLO renewals to emissions reporting — a checklist to keep your operations audit-ready.",
    id: "facility-compliance-deadlines",
    mediaTone: "slate",
    title:
      "Five Compliance Deadlines Every Facility Manager Should Track in 2026",
  },
  {
    ...articleDefaults,
    category: "Press Release",
    date: "24 Jun 2026",
    dateTime: "2026-06-24",
    description:
      "Hotel groups commit to audited efficiency programs targeting double-digit energy savings.",
    id: "hospitality-partners-mou",
    mediaTone: "navy",
    title: "EMI and Hospitality Partners Sign Energy Efficiency MoU",
  },
  {
    ...articleDefaults,
    category: "Energy News",
    date: "18 Jun 2026",
    dateTime: "2026-06-18",
    description:
      "How your electricity accounting choice changes your reported footprint — and what auditors expect.",
    id: "scope-2-accounting",
    mediaTone: "green",
    title: "Scope 2 Accounting: Market-Based vs Location-Based Methods",
  },
  {
    ...articleDefaults,
    category: "Insight",
    date: "11 Jun 2026",
    dateTime: "2026-06-11",
    description:
      "Co-firing demand is rising. Here's how reliable feedstock partnerships de-risk the transition.",
    id: "biomass-supply-chains",
    mediaTone: "blue",
    title: "Why Biomass Supply Chains Are the Next Decarbonization Frontier",
  },
];

export function getArticleHref(articleId: string) {
  return `/articles/${articleId}`;
}

export function getArticleById(articleId: string) {
  return articles.find((article) => article.id === articleId);
}

export function getRelatedArticles(articleId: string, limit = 3) {
  return articles.filter((article) => article.id !== articleId).slice(0, limit);
}

export const homeArticles = articles.slice(0, 5);
